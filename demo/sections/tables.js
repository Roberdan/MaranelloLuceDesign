...
export function createTablesSection() {
  ...
  section.innerHTML = `
    <style>
      ...
      #tables .mn-rich-table__filter{width:100%;padding:8px 10px;border:1px solid var(--mn-border);border-radius:999px;background:var(--mn-surface,transparent);color:var(--mn-text);font-size:var(--text-micro)}
...
function groupedRows(rows, state) { return GROUP_ORDER.map((group) => { const items = rows.filter((row) => row.group === group); if (!items.length) return ''; const open = !state.collapsed[group]; return `<tr class="mn-rich-table__group" data-group="${group}"><td colspan="${COLUMNS.length}">${open ? '▼' : '▶'} ${group.toUpperCase()} ${items.length}</td></tr>${open ? items.map((row) => `<tr class="mn-rich-table__row"><td><strong>${row.program}</strong><div class="mn-micro" style="color:var(--mn-text-muted)">${row.group}</div></td><td>${ownerCell(row)}</td><td><span class="mn-status mn-status--${statusTone(row.status)}"><span class="mn-status__dot"></span> ${row.status}</span></td><td>${qualityCell(row.quality)}</td><td><span class="mn-rich-table__badge mn-rich-table__badge--${row.impact.toLowerCase()}">${row.impact}</span></td><td>${row.region}</td></tr>`).join('') : ''}`; }).join(''); }
...
function ownerCell(row) { return `<div class="mn-rich-table__owner"><span class="mn-rich-table__avatar">${row.owner.split(' ').map((part) => part[0]).join('').slice(0, 2)}</span><div><strong>${row.owner}</strong><div class="mn-micro" style="color:var(--mn-text-muted)">${row.role}</div></div></div>`; }
