...
export function createSocialGraphSection() {
  ...
  section.innerHTML = `<div class="mn-container"><p class="mn-section-number">16 — Agent Graph</p><h2 class="mn-title-section" style="margin-bottom:var(--space-sm)">Maranello Luce Hub-and-Spoke Network</h2><p class="mn-body" style="margin-bottom:var(--space-xl)">28 fictional agents across orchestrator, executor, validator, researcher, and monitor teams.</p><details class="mn-code-snippet"><summary class="mn-label" style="cursor:pointer;color:var(--mn-accent);margin-bottom:var(--space-sm)">⟨/⟩ Usage</summary><pre class="mn-card-dark" style="padding:var(--space-md);font-family:var(--font-mono);font-size:var(--text-micro);overflow-x:auto;margin-bottom:var(--space-lg);border-left:3px solid var(--mn-accent)"><code>Maranello.socialGraph(el, {
  nodes: [{ id: 'a', label: 'Agent Opus', group: 'Orchestrators' }],
  edges: [{ source: 'a', target: 'b', weight: 2 }],
  onClick: (node) =&gt; console.log(node.label)
