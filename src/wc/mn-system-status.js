/**
 * <mn-system-status> — Web Component service health monitor
 * Compact pill + expandable panel showing service status with polling.
 *
 * @attr {string} services      - JSON [{name, url?}]
 * @attr {number} poll-interval - Auto-refresh interval in ms (default: 30000)
 * @attr {string} version       - Application version label
...
class MnSystemStatus extends HTMLElement {
  ...
  constructor() {
    ...
    this._onDocKey = (e) => { if (e.key === 'Escape' && this._isOpen) this._closePanel(); };
  }
  ...
  _readAttrs() {
    try { this._services = JSON.parse(this.getAttribute('services') || '[]'); }
    catch { this._services = []; }
  ...
  _renderResults() {
    ...
    const hasSlow = this._results.some((r) => r.ok && r.ms > 1000);
    const cls = hasDown ? 'danger' : hasSlow ? 'warning' : 'active';
    const label = hasDown ? 'Degraded Performance'
      : hasSlow ? 'Partial Degradation' : 'All Systems Operational';
  ...
  _startPolling() {
    ...
    const interval = isNaN(ms) ? 30000 : ms;
